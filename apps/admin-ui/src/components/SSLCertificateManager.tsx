...
export default function SSLCertificateManager({ domain, applicationId }: SSLCertificateManagerProps) {
  ...
  const fetchCurrentAdmin = async () => {
    try {
      ...
      if (admin && admin.email) {
        setRegistrationEmail(admin.email);
      }
    } catch (error) {
  ...
  const fetchCertificate = async () => {
    try {
      setLoading(true);
      const cert = await api.getCertificate(domain);
      setCertificate(cert);
  ...
  const fetchSiteConfig = async () => {
    try {
      const response = await api.request<any>('get', `/api/sites/${encodeURIComponent(domain)}`);
      if (response) {
  ...
  const updateSSLEnabled = async (enabled: boolean) => {
    try {
      await api.request('put', `/api/sites/${encodeURIComponent(domain)}`, {
        ssl_enabled: enabled
      });
  ...
  const generateLetsEncryptCert = async () => {
    ...
    try {
      setGenerating(true);
      setCertError(null);
      await api.generateLetsEncryptCertificate(domain, applicationId, registrationEmail);
  ...
  const uploadManualCertificate = async () => {
    ...
    try {
      await api.uploadCertificate(domain, {
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={async (e) => {
                          try {
                            await api.updateCertificateSettings(domain, { autoRenew: e.target.checked });
                            fetchCertificate();
