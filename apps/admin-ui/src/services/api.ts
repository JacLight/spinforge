...
class SpinForgeAPI {
  ...
  async metrics(): Promise<Metrics> {
    return this.request<Metrics>('get', '/api/metrics');
  }

  // New comprehensive metrics endpoints
  async systemMetrics(): Promise<SystemMetrics> {
    return this.request<SystemMetrics>('get', '/api/_metrics/all');
  }

  async dockerStats(): Promise<DockerStats> {
    return this.request<DockerStats>('get', '/api/_metrics/all');
  }

  async keydbMetrics(): Promise<KeyDBMetrics> {
    return this.request<KeyDBMetrics>('get', '/api/_metrics/all');
  }

  async serviceHealth(): Promise<ServiceHealth[]> {
    return this.request<ServiceHealth[]>('get', '/api/_metrics/all');
  }

  async allMetrics(): Promise<AllMetrics> {
    return this.request<AllMetrics>('get', '/api/_metrics/all');
  }

  async requestMetrics(): Promise<any> {
    return this.request('get', '/api/_metrics/requests');
  }

  async deploymentStats(): Promise<any> {
    return this.request('get', '/api/_metrics/deployments');
  }

  async getIdleMetrics(): Promise<IdleMetrics> {
    return this.request<IdleMetrics>('get', '/api/metrics');
  }

  async getIdleInfo(spinletId: string): Promise<IdleInfo> {
    return this.request<IdleInfo>('get', `/api/sites/${spinletId}/metrics`);
  }
  ...
  async getAllAdmins(): Promise<Array<{
    ...
    const response = await this.request<{ admins: any[], total: number }>('get', '/_admin/admins');
  ...
  async getCurrentAdmin() {
    return this.request('get', '/_admin/profile');
  }
  ...
  async updateProfile(data: {
    ...
    return this.request('put', '/_admin/profile', data);
  }
  ...
  async changePassword(data: {
    ...
    return this.request('post', '/_admin/profile/password', data);
  }
  ...
  async getCertificate(domain: string) {
    return this.request('get', `/_admin/certificates/${encodeURIComponent(domain)}`);
  }

  async generateLetsEncryptCertificate(domain: string, applicationId: string, email?: string) {
    return this.request('post', '/_admin/certificates/letsencrypt', { domain, applicationId, email });
  }

  async uploadCertificate(domain: string, data: {
    certificate: string;
    privateKey: string;
    chain?: string;
  }) {
    return this.request('post', `/_admin/certificates/${encodeURIComponent(domain)}/manual`, data);
  }

  async renewCertificate(domain: string) {
    return this.request('post', `/_admin/certificates/${encodeURIComponent(domain)}/renew`, {});
  }

  async deleteCertificate(domain: string) {
    return this.request('delete', `/_admin/certificates/${encodeURIComponent(domain)}`);
  }
