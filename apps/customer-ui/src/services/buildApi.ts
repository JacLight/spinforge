/**
 * SpinForge - SpinBuild API client (building-api), customer-ui flavor.
 *
 * Separate axios instance from services/api.ts because building-api lives at
 * a different origin (build.spinforge.dev) than hosting-api (admin/customer
 * UIs both call api.spinforge.dev for hosting). The customer-ui carries a
 * customer session token in localStorage.authToken (apitoken: / session: /
 * sfc_) which building-api validates against the shared KeyDB.
 *
 * Customer-facing methods (Pipelines / Builds / Actions / Artifacts) hit
 * /_api/customer/* — those routes force req.customerId server-side and
 * assert ownership on every fetch. The other methods on this object
 * (jobs, deployments, signing, sessions, runners, vault, policy) still
 * point at /api/* admin routes and are reachable only from admin-only
 * pages that aren't wired into the customer-ui sidebar.
 *
 * Unlike the hosting-api client we do NOT auto-redirect on 401 —
 * cross-origin 401 from building-api often means the service is still
 * rolling out or CORS isn't quite right; we let the page show the error.
 */
...
client.interceptors.request.use((cfg: InternalAxiosRequestConfig) => {
  // Customer session first; fall back to adminToken for the orphan admin-only
  // routes that haven't been wired off this client yet (build/*, etc.).
  const tok =
...
export const buildApi = {
  ...
  // customers + policy
  // customer-ui has no admin scope — stub the customer list so the
  // Builds/Artifacts customer-filter dropdown renders empty instead of 401-ing.
  listCustomers: async (): Promise<{ customers: BuildCustomer[]; total: number }> => ({
    customers: [], total: 0,
  }),
  ...
  // actions catalog (customer-scoped — read-only, identical content to admin)
  listActions: () =>
    client.get<{ count: number; actions: ActionDef[]; byCategory: Record<string, ActionDef[]> }>('/_api/customer/actions')
      .then(r => r.data),
  ...
  createPipeline: (body: {
    ...
    client.post<Pipeline>('/_api/customer/pipelines', body).then(r => r.data),
  listPipelines: (q: { customerId?: string; limit?: number; offset?: number } = {}) =>
    client.get<{ pipelines: Pipeline[]; total: number }>('/_api/customer/pipelines', { params: q }).then(r => r.data),
  getPipeline: (id: string) =>
    client.get<Pipeline>(`/_api/customer/pipelines/${encodeURIComponent(id)}`).then(r => r.data),
  updatePipeline: (id: string, patch: {
    ...
    client.put<Pipeline>(`/_api/customer/pipelines/${encodeURIComponent(id)}`, patch).then(r => r.data),
  deletePipeline: (id: string) =>
    client.delete<void>(`/_api/customer/pipelines/${encodeURIComponent(id)}`).then(r => r.data),
  validatePipeline: (stages: Stage[]) =>
    client.post<{ valid: boolean; errors: PipelineValidationError[] }>(
      '/_api/customer/pipelines/validate',
      { stages },
    ).then(r => r.data),

  // builds (customer-scoped — server forces customerId)
  createBuild: (body: { pipelineId: string; trigger?: any; inputs?: Record<string, any>; customerId?: string }) =>
    client.post<Build>('/_api/customer/builds', body).then(r => r.data),
  listBuilds: (q: { pipelineId?: string; customerId?: string; status?: string; limit?: number; offset?: number } = {}) =>
    client.get<{ builds: Build[]; total: number }>('/_api/customer/builds', { params: q }).then(r => r.data),
  getBuild: (id: string) =>
    client.get<Build>(`/_api/customer/builds/${encodeURIComponent(id)}`).then(r => r.data),
  cancelBuild: (id: string) =>
    client.post<Build>(`/_api/customer/builds/${encodeURIComponent(id)}/cancel`, {}).then(r => r.data),
  resumeBuild: (id: string) =>
    client.post<Build>(`/_api/customer/builds/${encodeURIComponent(id)}/resume`, {}).then(r => r.data),
  retryStage: (id: string, stageId: string, overrides?: Record<string, any>) =>
    client.post<Build>(
      `/_api/customer/builds/${encodeURIComponent(id)}/stages/${encodeURIComponent(stageId)}/retry`,
      overrides ? { with: overrides } : {},
    ).then(r => r.data),
  getBuildEvents: (id: string, limit = 200) =>
    client.get<{ buildId: string; events: BuildEvent[] }>(
      `/_api/customer/builds/${encodeURIComponent(id)}/events`, { params: { limit } },
    ).then(r => r.data),
  getStageDetail: (buildId: string, stageId: string) =>
    client.get<{ buildId: string; stage: Stage; events: BuildEvent[]; log: StageLogLine[] }>(
      `/_api/customer/builds/${encodeURIComponent(buildId)}/stages/${encodeURIComponent(stageId)}`,
    ).then(r => r.data),
  ...
  downloadArtifact: async (buildId: string, stageId: string, key: string, filenameHint?: string) => {
    const r = await client.get(
      `/_api/customer/builds/${encodeURIComponent(buildId)}/artifacts/${encodeURIComponent(stageId)}/${encodeURIComponent(key)}`,
      { responseType: 'blob' },
    );
