...
// that's why every /_api/customer/sites? request was going out without auth and
...
export const hostingAPI = {
  ...
  async listVHosts(params?: VHostSearchParams): Promise<VHost[]> {
    const response = await apiClient.get('/_api/customer/sites', { params });
  ...
  async getVHost(domainOrId: string): Promise<VHost> {
    ...
    const response = await apiClient.get(`/_api/customer/sites/${domain}`);
    return response.data;
  },

  // Create virtual host
  async createVHost(vhost: VHost): Promise<{ message: string }> {
    const response = await apiClient.post('/_api/customer/sites', vhost);
    return response.data;
  },
  ...
  async updateVHost(domainOrId: string, vhost: Partial<VHost>): Promise<{ message: string }> {
    ...
    const response = await apiClient.put(`/_api/customer/sites/${domain}`, vhost);
    return response.data;
  },
  ...
  async deleteVHost(domainOrId: string): Promise<{ message: string }> {
    const domain = domainOrId;
    const response = await apiClient.delete(`/_api/customer/sites/${domain}`);
    return response.data;
  },
  ...
  async getVHostLogs(domainOrId: string, params?: {
    ...
    const domain = domainOrId;
    const response = await apiClient.get(`/_api/customer/sites/${domain}/logs`, { params });
    return response.data;
  },

  // Upload zip file for static site
  async uploadStaticSiteZip(domain: string, formData: FormData): Promise<any> {
    const response = await apiClient.post(`/_api/customer/sites/${domain}/upload`, formData, {
      headers: {
  ...
  async getContainerStats(domain: string): Promise<any> {
    try {
      const response = await apiClient.get(`/_api/customer/sites/${domain}/container/stats`);
      return response.data;
    } catch (error) {
  ...
  async getContainerInfo(domain: string): Promise<any> {
    try {
      const response = await apiClient.get(`/_api/customer/sites/${domain}/container/info`);
      return response.data;
    } catch (error) {
  ...
  async execInContainer(domain: string, command: string): Promise<any> {
    try {
      const response = await apiClient.post(`/_api/customer/sites/${domain}/container/exec`, { command });
      return response.data;
    } catch (error) {
  ...
  async getContainerFiles(domain: string, path: string = '/'): Promise<any> {
    try {
      const response = await apiClient.get(`/_api/customer/sites/${domain}/container/files`, { 
        params: { path }
      });
      return response.data;
    } catch (error) {
  ...
  async getContainerLogs(domain: string, params?: {
    ...
    try {
      const response = await apiClient.get(`/_api/customer/sites/${domain}/container/logs`, { 
        params,
  ...
  async checkReadiness(domain: string): Promise<{
    ...
    try {
      const response = await apiClient.get(`/_api/customer/sites/${domain}/readiness`);
      return response.data;
    } catch (error: any) {
