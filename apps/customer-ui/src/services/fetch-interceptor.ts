...
const patchedFetch: typeof window.fetch = async (
  ...
  // customer-ui never auto-redirects on admin 401s. Many /api/* and
  // /_admin/* endpoints will 401 for a customer (insufficient scope, not
  // invalid session). Mirroring the axios interceptor: surface the error
  // inline; keep the customer session intact. The page handles the 401
  // however it likes (toast, empty state, etc).
  return response;
};
