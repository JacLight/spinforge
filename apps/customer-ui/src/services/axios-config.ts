...
apiClient.interceptors.response.use(
  ...
  (error: AxiosError) => {
    ...
    // customer-ui never auto-redirects on 401. Many admin endpoints will 401
    // for a customer (insufficient scope, not invalid session) — turning that
    // into a redirect causes a refresh loop because useEffect restores the
    // session token from `authToken` on every reload. The page surfaces the
    // error inline; the user stays logged in.
    if (error.response?.status === 401 && error.config?.url?.includes("/_api/customer/")) {
      // Genuine customer-auth failure → clear and bounce to login.
