...
function StaticSiteManagement({ vhost, isEditing, formData, setFormData }: { 
  ...
  const handleClearFiles = async () => {
    const ok = await confirm({
      title: 'Delete all static files?',
      description: `Removes every uploaded file under ${vhost.domain}. Vhost config and routing are preserved. This cannot be undone.`,
      severity: 'danger',
      confirmLabel: 'Delete all files',
      typeToConfirm: vhost.domain,
    });
    if (!ok) return;
