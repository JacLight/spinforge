...
export default function PipelineEditorDrawer({ isOpen, onClose, pipeline, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    buildApi.listCustomers()
      .then((r) => {
        if (canceled) return;
        const list = (r.customers || []).map((c: any) => ({ id: c.id, name: c.name }));
        // customer-ui has no admin scope so listCustomers returns []. Seed
        // the picker from the logged-in customer so create still works.
        if (list.length === 0) {
          const myId = localStorage.getItem('customerId');
          const myEmail = localStorage.getItem('customerEmail');
          if (myId) list.push({ id: myId, name: myEmail || undefined });
        }
    ...
    (async () => {
      ...
      try {
        ...
        if (pipeline) {
          ...
          // customer-ui only ever has one customer (the logged-in one), so
          // pre-fill from the session and the picker becomes effectively read-only.
