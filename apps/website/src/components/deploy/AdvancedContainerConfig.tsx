...
export default function AdvancedContainerConfig({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Basic validation - just check if it mentions services
      // The backend will do proper YAML validation
      if (!text.includes('services:')) {
        setImportError('File does not appear to be a docker-compose file (missing services section)');
        return;
      }
      ...
      // If there's an import handler, let the parent handle validation
      if (onImportCompose) {
        onImportCompose(text);
      }
    } catch (error: any) {
