...
export default function Applications() {
  ...
  // Remove customer filter for customer view
  ...
  // Fetch vhosts with error handling
  const {
    data: vhostsData = [],
    isLoading,
    error,
  ...
  // Filter vhosts locally based on search and type
  const vhosts = vhostsData.filter((vhost: VHost) => {
    const matchesSearch = !debouncedSearchTerm || 
      vhost.domain?.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      vhost.customerId?.toLowerCase().includes(debouncedSearchTerm.toLowerCase());
    
    const matchesType = !selectedType || vhost.type === selectedType;
    
    return matchesSearch && matchesType;
  });

  const allVhosts = vhostsData;
  ...
  // Show error only if it's an actual network error, not just empty data
  if (error && !(vhostsData && vhostsData.length === 0)) {
    return (
          ...
          >
            ...
            <AnimatePresence>
              {showFilters && (
                ...
                >
                  {/* Customer filter removed for customer view */}
                    ...
                    return (
                      <tr key={vhost.domain} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap flex gap-2 items-center">
                          {getStatusIcon(vhost)}
                             <Link className="text-xs font-medium text-gray-900 ml-2" href={'/dashboard/applications/' + vhost.domain}>
                          Open
                        </Link>
                        </td>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-3 border-t">
                      ...
                      {(vhost.type === "container" ||
                        ...
                        <button
                          onClick={() =>
                            router.push(`/dashboard/containers/${vhost.domain}`)
                          }
