...
prompt() {
  ...
  # "Explicitly set to empty string" is a valid answer for fields like
  # KEYDB_PEERS / CONSUL_PEERS / NOMAD_PEERS where empty means
  # "standalone, no peers". ${var+SET} returns non-empty iff the var is
  # defined, regardless of whether its value is empty.
...
# ─── .env ─────────────────────────────────────────────────────────────
# Always regenerate so re-running the script with different env vars
# (NODE_IP, KEYDB_PEERS, etc.) actually changes the config. Existing
# .env is backed up first — manual edits are not silently clobbered,
# the operator can diff the backup if they had secrets (e.g. AWS keys)
# they need to carry across.
ENV_FILE="$SPINFORGE_DIR/.env"
if [ -f "$ENV_FILE" ]; then
  BAK="$ENV_FILE.bak.$(date +%s)"
  cp -a "$ENV_FILE" "$BAK"
  log "existing .env backed up to $BAK"
fi
