...
export class HotDeploymentWatcher {
  ...
  async start(): Promise<void> {
    try {
      ...
      this.logger.error(`Deployment directory not found: ${this.deploymentPath}`);
      return;
    }
    
    this.logger.info(`Starting hot deployment watcher on ${this.deploymentPath}`);
    
    // Initial scan of existing deployments
    await this.scanDeployments();
    
    // Watch for new deployments
    this.watcher = watch(this.deploymentPath, { recursive: true }, async (eventType, filename) => {
      if (!filename) return;
      
      // Check if it's a deployment trigger
      if (this.isDeploymentTrigger(filename)) {
        const deploymentDir = join(this.deploymentPath, filename.split('/')[0]);
  ...
  private isDeploymentTrigger(filename: string): boolean {
    ...
    return filename.endsWith('deploy.yaml') || 
  ...
  private async processDeployment(deploymentPath: string): Promise<void> {
    ...
    try {
      ...
      if (!validation.valid) {
        this.logger.error(`Invalid deployment config for ${deploymentId}`, { errors: validation.errors });
        return;
      }
      ...
      this.logger.error(`Failed to process deployment ${deploymentId}`, { error });
  ...
  private async loadDeploymentConfig(deploymentPath: string): Promise<DeploymentConfig | null> {
  ...
  private async deploy(config: DeploymentConfig, buildPath: string): Promise<void> {
    const domains = Array.isArray(config.domain) ? config.domain : [config.domain];
    ...
    if (config.build?.command) {
      await this.runBuildCommand(config.build.command, buildPath, config.build.env);
    }
  ...
  private async runBuildCommand(command: string, cwd: string, env?: Record<string, string>): Promise<void> {
  ...
  private async createMarker(path: string, filename: string, content: any): Promise<void> {
