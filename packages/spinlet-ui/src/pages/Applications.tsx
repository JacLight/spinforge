...
export default function Applications() {
  ...
  const { data: routes = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const routeData = selectedCustomer === 'all' 
        ...
        const routesWithStatus = await Promise.all(routeData.map(async route => {
          ...
          if (route.framework === 'static') {
            try {
              ...
              console.warn(`Failed to verify deployment for ${route.domain}`, e);
            }
          }
                  ...
                  <tr key={route.domain} className="hover:bg-gray-50 transition-colors">
                          ...
                          >
                            ...
                            <div className="text-xs text-gray-500">
                              <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                                route.framework === 'static' ? 'bg-green-100 text-green-800' :
                                route.framework === 'reverse-proxy' ? 'bg-purple-100 text-purple-800' :
                                'bg-gray-100 text-gray-800'
                              }`}>
                                {route.framework}
                              </span>
                              {route.framework === 'reverse-proxy' && route.config?.proxy?.target && (
                                <span className="ml-2 text-purple-600 font-mono text-xs">→ {route.config.proxy.target}</span>
                              )}
                            </div>
                          </button>
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-mono text-gray-900">
                        {route.framework === 'static' ? (
                          ...
                          <span className="text-purple-600">{route.config?.proxy?.target || 'No target'}</span>
                        ) : (
                          <div className="group relative inline-block">
                            <span className="cursor-help">{route.spinletState?.servicePath || '-'}</span>
                            {route.spinletState?.servicePath && (
                              <div className="absolute left-0 bottom-full mb-2 w-64 p-2 bg-gray-800 text-white text-xs rounded shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                <p className="text-gray-300 mb-1">Internal spinlet URL:</p>
                                <code className="text-yellow-400">http://{route.spinletState.servicePath}</code>
                        ...
                        {route.deploymentStatus === 'failed' ? (
                          <div className="group relative">
                            ...
                            {route.deploymentError && (
                              <div className="absolute left-0 bottom-full mb-2 w-80 p-3 bg-red-900 text-white text-xs rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                <p className="font-medium text-red-200 mb-1">Deployment Error:</p>
                                <p className="text-red-100">{route.deploymentError}</p>
                                <div className="absolute bottom-0 left-4 transform translate-y-1/2 rotate-45 w-2 h-2 bg-red-900"></div>
                              </div>
                            )}
                          </div>
                        ) : route.framework === 'static' ? (
                          route.verificationStatus && !route.verificationStatus.accessible ? (
                            <div className="group relative">
                              <div className="flex items-center">
                                <div className="h-2 w-2 bg-red-400 rounded-full"></div>
                                <span className="ml-2 text-sm text-gray-600">Not Found</span>
                              </div>
                              <div className="absolute left-0 bottom-full mb-2 w-80 p-3 bg-red-900 text-white text-xs rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                <p className="font-medium text-red-200 mb-1">Deployment Error:</p>
                                <p className="text-red-100">{route.verificationStatus.error || 'Deployment not accessible'}</p>
                                <p className="text-red-200 mt-1">Status: {route.verificationStatus.status}</p>
                                <div className="absolute bottom-0 left-4 transform translate-y-1/2 rotate-45 w-2 h-2 bg-red-900"></div>
                              </div>
                            </div>
                          ) : (
                          ...
                          route.spinletState.state === 'running' ? (
                            ...
                            <>
                              ...
                              <span className="ml-2 text-sm text-gray-600">{route.spinletState.state}</span>
                            </>
                          )
                        ) : (
                          <>
                            <div className="h-2 w-2 bg-orange-400 rounded-full"></div>
                            <span className="ml-2 text-sm text-gray-600">Not Deployed</span>
                          </>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {route.spinletState?.lastAccess ? (
                        <div className="text-sm text-gray-600">
                          {formatDeploymentTime(new Date(route.spinletState.lastAccess).toISOString())}
                          {route.spinletState.state === 'running' && (
                    ...
                    {/* Idle Timeout */}
                    <td className="px-6 py-4 whitespace-nowrap">
                      {route.framework === 'static' || route.framework === 'reverse-proxy' ? (
                        ...
                        <div className="relative group">
                          ...
                          <div className="absolute right-0 bottom-full mb-2 w-72 p-3 bg-gray-900 text-white text-xs rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                            <div className="space-y-2">
                              <div>
                                <p className="font-medium text-gray-300 mb-1">External URL:</p>
                                <code className="text-green-400">http://{route.domain}:9006</code>
                              </div>
                              <div className="border-t border-gray-700 pt-2">
                                <p className="font-medium text-gray-300 mb-1">Internal Docker URLs:</p>
                                <div className="space-y-1">
                                  <div>
                                    <span className="text-gray-400">From containers:</span>
                                    <code className="block text-blue-400">http://spinforge-hub:8080</code>
                                    <span className="text-gray-500">Host: {route.domain}</span>
                                  </div>
                                  <div className="mt-2">
                                    <span className="text-gray-400">From host:</span>
                                    <code className="block text-blue-400">http://localhost:9004</code>
                                    <span className="text-gray-500">Host: {route.domain}</span>
                                  </div>
                                </div>
                              </div>
